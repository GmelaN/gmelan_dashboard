...
public class NoticeService {
    ...
    public List<Notice> getNotice() {
        ...
        noticeList.sort(new Comparator<Notice>() {
            ...
            public int compare(Notice notice1, Notice notice2) {
                ...
                return -1 * notice1.getPublishedAt().compareTo(notice2.getPublishedAt());
            }
        });
    ...
    private List<Notice> getJinjuNotices() {
        String url = "https://young.jinju.go.kr/young/board/notice/list/0";
        List<Notice> jinjuNoticeList = new ArrayList<>();
        ...
        try {
            ...
            for (Element noticeElement : noticeElements) {
                ...
                if(date.after(fetchStartDate) || date.equals(new Date(0))) {
                    ...
                    noticeEntity.setUrl(link);
                    noticeEntity.setPublishedAt(Timestamp.from(date.toInstant()));
                    noticeEntity.setAgency("진주청년플랫폼");
                    jinjuNoticeList.add(noticeEntity);
                }
            }
    ...
    private List<Notice> getGNUNotices() {
        List<Notice> GNUNoticeList = new ArrayList<>();
        ...
        for(String urlString : URL) {
            ...
            for (Element noticeElement : noticeElements) {
                ...
                if(date.after(fetchStartDate) || date.equals(new Date(0))) {
                    ...
                    noticeEntity.setTitle(title);
                    noticeEntity.setUrl(link);
                    noticeEntity.setContent(title);
                    noticeEntity.setAgency(author);
                    noticeEntity.setPublishedAt(Timestamp.from(date.toInstant()));
                    noticeEntity.setFetchedAt(Timestamp.from(date.toInstant()));
                    GNUNoticeList.add(noticeEntity);
                }
            }
        }
